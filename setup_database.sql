...
SET DB_NAME = REPLACE(REPLACE(CURRENT_USER(), '.', '_'), '-', '_') || '_FINANCIAL_AI_HANDSON';
...
-- リポジトリの内容確認
EXECUTE IMMEDIATE 'LIST @' || $DB_NAME || '.RAW.GIT_FINANCIAL_HANDSON/branches/main';
...
-- コピーされたファイルの確認
EXECUTE IMMEDIATE 'LIST @' || $DB_NAME || '.RAW.HANDSON_RESOURCES/csv/';
...
EXECUTE IMMEDIATE 'SELECT COUNT(*) AS TOTAL_RECORDS FROM ' || $DB_NAME || '.RAW.LOAN_MASTER';
EXECUTE IMMEDIATE 'SELECT * FROM ' || $DB_NAME || '.RAW.LOAN_MASTER LIMIT 10';
...
EXECUTE IMMEDIATE 'SELECT COUNT(*) AS TOTAL_RECORDS FROM ' || $DB_NAME || '.ANALYTICS.EXECUTION_DATA';
EXECUTE IMMEDIATE 'SELECT * FROM ' || $DB_NAME || '.ANALYTICS.EXECUTION_DATA LIMIT 10';
...
EXECUTE IMMEDIATE 'SELECT COUNT(*) AS TOTAL_RECORDS FROM ' || $DB_NAME || '.ANALYTICS.LOAN_PORTFOLIO';
EXECUTE IMMEDIATE 'SELECT * FROM ' || $DB_NAME || '.ANALYTICS.LOAN_PORTFOLIO LIMIT 10';
